...
The hooks are the only tested code. Each `hooks/<name>.sh` has a `hooks/<name>.test.sh` beside it: a plain bash table test, no runner.

```sh
for t in hooks/*.test.sh; do bash "$t" || break; done
```

Each file prints `N passed, M failed` and exits non-zero on any failure.
