...
- **D1 is Cloudflare's SQLite → you own it** (schema, migrations via `wrangler d1 migrations`, queries), **not `postgres-architect`** (which owns Postgres). If the app needs Postgres from a Worker, that's **Hyperdrive** in front of `postgres-architect`'s DB — coordinate on the seam. For D1 schema, migration or query work, load **`sql`** (modeling, NULL semantics, expand/contract), then **`sqlite`** on its D1 branch.
