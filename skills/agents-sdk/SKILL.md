...
<!-- vendored verbatim from `cloudflare/skills` main:skills/agents-sdk (Apache-2.0; LICENSE kept).
     the `agents` package playbook; the `cloudflare` skill routes here and no longer carries references/agents-sdk itself.
     re-sync: re-download skills/agents-sdk/ from the cloudflare/skills repo. -->
