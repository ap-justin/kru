...
- **Severity is reach × harm, not capability.** The same bug is `high` on an anonymous endpoint and a non-event behind admin auth. A security finding carries gain · incidence · precondition (`code-reviewer`'s incentive bullet), and `security.md` is the build side of the same arithmetic.
