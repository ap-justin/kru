...
Required on the same 25 seats as Block N — every seat that **writes code or config**. Reviewers and the five text-producing seats don't build, so there is no slice to read back; `code-reviewer` carries the **read side** instead (below).
