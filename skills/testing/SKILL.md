...
- **Assert something that can fail.** The check before you keep a test: would it still pass if every function it imports returned `undefined`? Then it observes no behavior. The shapes that stay green under that check: `toBeDefined()`, a bare `not.toThrow()`, or a snapshot nobody read; only `toHaveBeenCalled` or an empty result (`toEqual([])`, `toHaveLength(0)`); an assertion reading data the test built in `beforeEach` while the subject never runs in the body. Call the subject inside the test with one concrete input and assert the literal value or effect the caller depends on — for a mock, the payload it received, not that it was called. No such assertion exists → delete the test.
- **A test that something does *not* happen ships beside one proving the same read is live.** "Scheduling moves no score" passes identically whether the rule holds or the read is broken, stubbed, or looking at the wrong table — pair it with a case where a real event moves that exact read. It is the negative case that usually ships unguarded.
